...
ESC_FORWARD = 330   # PCA9685 ticks ≈ 1611 µs  (demo creep speed)
...
def main():
    ...
    try:
        while True:
            ...
            # ── Throttle ──────────────────────────────────────────────────
            target_throttle = state_to_throttle(final_state)
            ...
            if state_changed or (loop_count % PRINT_EVERY_N_LOOPS == 0):
                ...
                print(
                    ...
                    f"v2i={v2i_state:7s} live={str(v2i_live):5s} "
                    f"final={final_state:7s} src={decision_source:18s} | "
                    f"rem={v2i_remaining:.2f}s conf={result.confidence:.2f} | "
                    f"lane={off_str}px steer={steer_ticks}tks | "
                    f"{'FWD' if target_throttle == ESC_FORWARD else 'STOP'}"
                )
